...
/**
 * @typedef NaivebayesOptions
 * @param {function(string): Array<string>} tokenizer custom tokenization function 
 * @param {boolean} returnAllResults Return all results in the categorize method.
 */
 ...
 *
 * @param {NaivebayesOptions} options
 */
 ...
 *
 * @param  {String} text
 * @return {Promise<string>|Promise<Array<CategorizeResult>>} category
 */
Naivebayes.prototype.categorize = async function (text) {
  let chosenCategories = await categorize(this, text)
  if(this.options.returnAllResults == null || !this.options.returnAllResults) {
    let chosenCategory = chosenCategories.reduce(function(result, entry) {
      if(result == null) {
        return entry
      }else if(entry.probability > result.probability) {
        return entry
      }else {
        return result
      }
    }, null)
    return chosenCategory != null ? chosenCategory.category : null
  }else {
...
async function categorize(self, text) {
  ...
  .forEach(function (category) {
    ...
    chosenCategories.push({category: category, probability: logProbability})
  })
